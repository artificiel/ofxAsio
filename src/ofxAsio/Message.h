...
enum MessageType : uint16_t {
	...
	JSON = First_User_Slot + 2,
	MeshVector = First_User_Slot + 3
	//
	//--
};
...
struct MeshVector {
	struct {
		...
		MessageType messageType = MessageType::MeshVector; // 4 bytes here
...
//	Message(const std::vector<std::unique_ptr<ofMesh>> &);
	...
	void setData(const std::vector<std::unique_ptr<ofMesh>> &);
	...
	bool getData(std::vector<std::unique_ptr<ofMesh>> &) const;
